...
<head>
    ...
    $.pf_defaultparams = function(a_obj){
		let rtn_params;
		if(a_obj.attr('id') == "dg0") rtn_params = {CRS_ID:$.jf_seqdgdata('/si/SI0503G0K0', 'post')}
    	return rtn_params;
    }
	...
	$.pf_acceptcfmsg = function(a_type){
		if(a_type == 'save'){
			if($.jf_validatedata(null, $('#ef0'), $.jf_fnddgstrct($('#dg0')), 'f') ){
				$.jf_savedgdata($('#ef0'), '/si/SI0503G0S0', 'post', null)
			}
			else
	...
	$.pf_childparams = function(a_obj, a_row){
		let rtn_params;
		if(a_obj.attr('id') == 'dg1') rtn_params = {CRS_ID : a_row.CRS_ID}
		return rtn_params;
	}
