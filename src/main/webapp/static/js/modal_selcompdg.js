$(function(){
  ...
  $('#selcomp_layout0').layout('panel','center').append('<table id="selcomp_dg0" class="easyui-datagrid" style="width:100%;height:100%"></table>');
	...
	/*modal page 함수*/	
	$.mf_selcompmdopendg = function(a_dg, a_form, a_values, a_rtnobj){
		let v_win = $('#selcomp');
			$.jf_modmdstrct(v_win, a_form, a_values, a_rtnobj, a_dg);
	...
	$.mf_selcompmdclose = function(a_row){
		...
		let v_dg = js_mdstrct[v_idx].datagrid;
		...
		//중복체크
		if(!$.jf_dupcheck($('#'+v_dg), 'COMP_ID', v_values.COMP_ID)) $.jf_append($('#'+v_dg), v_values);
		else $.tracomalmsg('정보', '데이터가 중복되어 입력할 수 없습니다.', null);

		//$.jf_append($('#'+v_dg), v_values);
