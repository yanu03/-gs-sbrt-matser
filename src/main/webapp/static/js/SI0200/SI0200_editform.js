...
$(function(){
	$('#ef0').form({
        ...
        onChange: function(target){
            ...
            switch(target.id){
                ...
                case "USE_YN_N":
                        if($(target).radiobutton('options').checked){a_vals = $.jf_singledatatojson('USE_YN', $(target).radiobutton('options').value);} 
                    break;
    ...
    $('#fm_panel0').append('<input id="RELS_DT" class="tracom-datebox" name="RELS_DT"><p>');
    ...
    $('#AREA').combobox({
		...
		height: 24,
		editable: false,
		url: 'http://192.168.34.18:8183/common/selectCommonDtlList',
		method: 'post',
    ...
    $('#sch_comp_btn').bind('click', function(){
        ...
        $.mf_selcompmdopen($('#ef0'), v_values, $('#VHC_NO'));
    });
    ...
    $('#USE_YN_N').radiobutton({
        ...
        onChange: function(checked){
            if(!jv_rowclick) return false;
            // if(checked == true) {
            //     // db에서 이 차량이 사용되고 있는지 확인해야함.
            //     $.tracomalmsg('정보','이 차량은 사용되고 있습니다.');
