...
$(function(){
	...
	$('#btn2').bind('click', function(){
		//미완료, 삭제시 데이터검증 필요함.
		$.jf_checkforeigntable($.jf_curdgrow($('#dg0')),"SI0501", function(a_type){
	...
	$('#btn4').bind('click', function(){
		 if($.jf_validatedata(null, $('#ef0'), $.jf_fnddgstrct($('#dg0')), 'f'))
		 {
			$.jf_savedgdata($('#dg0'), '/si/SI0501G0S0', 'post', null)
		 }
	});
  ...
  $('#btn7').bind('click', function(){
        $.jf_exceldownload($('#dg0'), '/si/SI0501G0_exlDownload?param='+ $('#sch_sb0').searchbox('getValue'));
    });
    ...
    $("#excelinputfile").on("change", function(e){
        ...
        $.jf_excelupload($('#dg0'), formData, '/si/SI0501G0_exlUpload');
    }); 
});