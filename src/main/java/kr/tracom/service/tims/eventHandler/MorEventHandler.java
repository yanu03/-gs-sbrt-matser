...
public class MorEventHandler{
	...
	private int getRemainTm(Map<String, Object> node, Map<String, Object> eventInfo, String nextSttnId) {
		double second = 0.0D;
		List nodeList = getNodeList(eventInfo);
		if (nodeList == null)
			return 0;
		for (int i = 0; i < nodeList.size(); i++) {
			...
			if (CommonUtil.stringToDouble(node2.getOrDefault("NODE_SN", 0).toString()) > curNodeSn) {
				...
				if ((nextSttnId != null) && (node2.get("NODE_ID") != null)
						&& (nextSttnId.equals(node2.get("NODE_ID")))) {
					return (int) second;
				}
			}
		}
	 ...
	 * curInfoMapper.selectVhcInfo(paramMap); g_vhcInfoMap.put(impID, vhcInfo); return
	 * vhcInfo; }
	 * 
	 * 
	 * if ((impID != null) && (impID.isEmpty() == false)) { vhcInfo = (Map<String,
	 * Object>)g_vhcInfoMap.get(impID); if ((vhcInfo != null)) { return vhcInfo; }
	 * else { vhcInfo = curInfoMapper.selectVhcInfo(paramMap); g_vhcInfoMap.put(impID,
	...
	private Map<String, Object> getVhcSttnInfo(Map<String, Object> paramMap) {
		...
		String sttnLinkId = curInfoMapper.selectSttnLinkId(paramMap);
			...
			for (int i = 0; i < request.getAttrCount(); i++) {
				...
				case BrtAtCode.BUS_INFO: // 정주기 버스 정보
					...
					try {
			            ...
			            wsDataMap.put("IMP_ID", busInfoMap.get("IMP_ID"));
			            wsDataMap.put("NODE_NM", busInfoMap.get("CUR_NODE_NM"));
			            wsDataMap.put("ETA", busInfoMap.get("ETA"));
				...
				case BrtAtCode.BUS_ARRIVAL_INFO: // 차량 도착정보
					...
					try {
						...
						for (AtBusArrivalInfoItem arrivalInfoItem : arrivalInfoList) {
							...
							arrivalInfoMap.put("ROUT_NM", (String)routNm.get("ROUT_NM"));
							...
							arrivalInfoMap.put("BUS_ID", arrivalInfoItem.getBusId());
				...
				case BrtAtCode.BUS_OPER_EVENT: // 운행 이벤트 정보
					...
					try {
						...
						try {
							...
							logger.error("BUS_OPER_EVENT Exception {}", e);
						}
						...
						{ // dashboard
				            ...
				            wsDataMap.put("IMP_ID", busEventMap.get("IMP_ID"));
				            wsDataMap.put("ETA", busEventMap.get("ETA"));							
	...
	private int setOperEventData(Map<String, Object> operEventMap) {
		...
		try {
			...
			if (curSttnInfo != null) {
		        operEventMap.put("ETA", Integer.valueOf(getRemainTm(realNodeInfo, operEventMap, (String)curSttnInfo.get("NEXT_STTN_ID"))));
