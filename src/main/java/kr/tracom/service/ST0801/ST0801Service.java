package kr.tracom.service.ST0801;
...
import kr.tracom.mapper.ST0801.ST0801Mapper;
import kr.tracom.support.ServiceSupport;

@Service
public class ST0801Service extends ServiceSupport {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(ServiceSupport.class);

	@Autowired
	private ST0801Mapper st0801Mapper;
	
	public List ST0801G0R0() throws Exception {
		Map<String, Object> map = getSimpleDataMap("dma_search");
		return st0801Mapper.ST0801G0R0(map);
	}
	
	public List ST0801G1R0() throws Exception {
		Map<String, Object> map = getSimpleDataMap("dma_sub_search");
		return st0801Mapper.ST0801G1R0(map);
	}
	
	public List ST0801G1R1() throws Exception {
		Map<String, Object> map = getSimpleDataMap("dma_sub_search");
		String temp[] = map.get("CRS_ID").toString().replace("[","").replace("]","").replace(" ","").split(",");
		map.put("CRS_ID", temp);
		return st0801Mapper.ST0801G1R1(map);
	}
	
	public List ST0801G2R0() throws Exception {
		Map<String, Object> map = getSimpleDataMap("dma_sub_search");
		String temp[] = map.get("CRS_ID").toString().replace("[","").replace("]","").replace(" ","").split(",");
		map.put("CRS_ID", temp);
		return st0801Mapper.ST0801G2R0(map);
	}
