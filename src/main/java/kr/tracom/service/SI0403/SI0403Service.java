package  kr.tracom.service.SI0403;
...
import kr.tracom.mapper.SI0403.SI0403Mapper;
...
public class SI0403Service extends ServiceSupport {
	...
	public Map SI0403G1S0() throws Exception {
		...
		List<Map<String, Object>> param = getSimpleList("dlt_BMS_ROUT_LINK_CMPSTN");
		try {
			for (int i = 0; i < param.size(); i++) {
				Map data = (Map) param.get(i);
				String rowStatus = (String) data.get("rowStatus");
				if (rowStatus.equals("U")) {
					iCnt += SI0403Mapper.SI0403G1U0(data);
				}
			}			
		} catch(Exception e) {
