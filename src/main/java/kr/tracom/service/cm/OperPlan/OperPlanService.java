package kr.tracom.service.cm.OperPlan;
...
public class OperPlanService extends ServiceSupport {
	...
	public List<Map<String, Object>> selectOperPlanRoutList() throws Exception {
		...
		Map<String, Object> operPlanMst = getSimpleDataMap("dma_BMS_OPER_PL_MST");
		...
		while(operSn<1000) {
			...
			reuturnMap.put("ROUT_GRP", param.get("ROUT_GRP"));
	 ...
	 * @throws Exception
	 */
	private List makeOperAllocPlNodeInfo(String allocId, String routId, String dayDiv, int operSn, boolean bSave
		...
		for(Map<String, Object> nodeInfo : nodeList) {
			int temp_node_sn = Integer.valueOf(nodeInfo.get("NODE_SN").toString());
			for(Map<String, Object> nextNodeInfo : nodeList) {
				int temp_next_node_sn = Integer.valueOf(nextNodeInfo.get("NODE_SN").toString());
				if(temp_next_node_sn > temp_node_sn) {
		...
		while(!isDone) {
			...
			for(Map<String, Object> nodeInfo : nodeList) {
				...
				if(nodeInfo.get("NEXT_NODE_SN")==null) {
					next_node_id = "";
					next_node_sn = -1;
					next_node_type = "";
				}
				else {
					next_node_id = String.valueOf(nodeInfo.get("NEXT_NODE_ID"));
					next_node_sn = Integer.valueOf(nodeInfo.get("NEXT_NODE_SN").toString());
					next_node_type = String.valueOf(nodeInfo.get("NEXT_NODE_TYPE"));
				}
				...
				if(node_sn == route_first_node_sn) {      //노선의 첫번째 노드이면
					...
					if(node_type.equals(Constants.NODE_TYPE_BUSSTOP)) { //정류장
						...
						if(chgType != OperPlanCalc.CHG_TYPE_MODIFY) { //수정인 경우에는 정류장 출도착 시각을 전달받음
							...
							stop_sec_peak = CommonUtil.bigDecimalToInt(nodeInfo.get("STOP_TM_PEAK"));
							stop_sec_none_peak =  CommonUtil.bigDecimalToInt(nodeInfo.get("STOP_TM_NONE_PEAK"));
				...
				if(node_sn != route_last_node_sn) {
					...
					if(next_node_type.equals(Constants.NODE_TYPE_CROSS) && !StringUtils.isEmpty(next_cross_id)){ //다음 노드가 교차로이면
						...
						sig_ctr_type = String.valueOf(nodeInfo.get("SIG_CTR_TYPE"));


						//현시정보 확인이 가능한 경우 [[
						if(!StringUtils.isEmpty(sig_ctr_type)){

							if(CommonUtil.notEmpty(nodeInfo.get("ENT_PHASE_NO_1"))) {
								enter_phase_no1 = CommonUtil.bigDecimalToInt(nodeInfo.get("ENT_PHASE_NO_1"));
							}
							if(CommonUtil.notEmpty(nodeInfo.get("ENT_PHASE_NO_3"))) {
								enter_phase_no2 = CommonUtil.bigDecimalToInt(nodeInfo.get("ENT_PHASE_NO_2"));
							}
							if(CommonUtil.notEmpty(nodeInfo.get("ENT_PHASE_NO_3"))) {
									enter_phase_no3 = CommonUtil.bigDecimalToInt(nodeInfo.get("ENT_PHASE_NO_3"));	
							}
							...
							if(enter_phase_no1 != 0) {
								...
								if(chgType == OperPlanCalc.CHG_TYPE_CHG_OPER) { //변경운행 생성 시에는 신호 고려
									...
									phase_remain_sec = operPlanMapper.selectPhaseRemainTm(paramMap); //신호 고려
								}
								...
								if(enter_phase_no2 != 0) {
									...
									if (phase_remain_sec <= 0) {
										...
										if(chgType == OperPlanCalc.CHG_TYPE_CHG_OPER) { //변경운행 생성 시에는 신호 고려
											...
											phase_remain_sec_tmp = operPlanMapper.selectPhaseRemainTm(paramMap); //신호 고려
										}
	 ...
	 * 운행계획 생성 
	 */
	public List makeOperAllocPlNodeInfo(String allocId, String routId, String dayDiv, int operSn, boolean bSave) {
		
		List operPlList = null;
		
		try {
			operPlList = makeOperAllocPlNodeInfo(allocId, routId, dayDiv, operSn, bSave, 0, 0, 0, null, null, null, null, null, null, OperPlanCalc.CHG_TYPE_NONE);
	...
	public List makeChgOperAllocPlNodeInfo(String allocId, String vhcId, String routId, String operDt, int operSn
		...
		try {
			...
			chgPlList = makeOperAllocPlNodeInfo(allocId, routId, dayDiv, operSn, false, stNodeSn, 0, offsetTm, timeMin, timeMax, null, null, null, null, OperPlanCalc.CHG_TYPE_CHG_OPER);
