package kr.tracom.service.cm.Intg;
...
public class IntgService extends ServiceSupport {
	...
	public List<Map<String, Object>> getIntgInfo() throws Exception {
		List<Map<String, Object>> jsonList = null;

		String intgKind = getDataString("INTG_KIND");
		String param = getParamString();
		String apiUrl = apiGatewayUrl + "local/getIntgInfo?" + param;
		...
		try {
			...
			try {
				...
				try {
					...
					jsonList = gson.fromJson(result, resultType);
				} catch (Exception e) {
					//logger.error("error");
