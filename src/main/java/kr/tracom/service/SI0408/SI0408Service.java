package  kr.tracom.service.SI0408;
...
import kr.tracom.mapper.SI0408.SI0408Mapper;
...
public class SI0408Service extends ServiceSupport {
	...
	public Map SI0408G1S0() throws Exception {
		...
		List<Map<String, Object>> routNodeListByRoutGrp = null;
		...
		try {
			...
			if(param.size()>0) { //동일 대표노선의 다른 노선 갱신 용 삭제해도 됨
				map2.put("ROUT_GRP",param.get(0).get("ROUT_GRP"));
				routList = routMapper.selectRoutListByRoutGrp(map2);
				map2.put("ROUT_ID",param.get(0).get("ROUT_ID"));
				map2.put("WAY_DIV",param.get(0).get("WAY_DIV"));
				routNodeListByRoutGrp = SI0408Mapper.selectRoutNodeAllByRoutGrp(map2);
			}
			
			if(routNodeListByRoutGrp!=null) { //동일 대표노선의 다른 노선 갱신 용 삭제해도 됨 //동일한 대표노선의 다른 노선의 정보 삭제
				String oldRoutId = ""; 
				for(Map<String, Object> routNode : routNodeListByRoutGrp) {
					/*String nodeId = "";
					String edNodeId = "";*/
			...
			/*if(routNodeListByRoutGrp!=null) { //동일 대표노선의 다른 노선 갱신 용 삭제해도 됨 //동일한 대표노선의 다른 노선의 정보 갱신
				String oldRoutId = "";
				String routId = "";
